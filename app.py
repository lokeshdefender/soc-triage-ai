...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Share+Tech+Mono&family=JetBrains+Mono:wght@400;500&family=Inter:wght@300;400;500;600&display=swap');
...
">
    <div style="
        ...
        background: linear-gradient(90deg, transparent, #1A6BAA, #00B4D8, #1A6BAA, transparent);
    "></div>
    <div style="display: flex; align-items: center; gap: 1.25rem; flex-wrap: wrap;">
        ...
        <div style="flex:1;">
            ...
            ">AI-powered first-pass security alert analysis · Llama 3.3 70B via Groq</div>
        </div>
        <div style="
            font-family: 'JetBrains Mono', monospace;
            font-size: 12px;
...
SEVERITY_CONFIG = {
    "CRITICAL": {"icon": "🔴", "color": "#F87171", "bg": "rgba(248,113,113,0.07)", "border": "rgba(248,113,113,0.35)"},
    "HIGH":     {"icon": "🟠", "color": "#FB923C", "bg": "rgba(251,146,60,0.07)",  "border": "rgba(251,146,60,0.35)"},
    "MEDIUM":   {"icon": "🟡", "color": "#FBBF24", "bg": "rgba(251,191,36,0.07)",  "border": "rgba(251,191,36,0.35)"},
    "LOW":      {"icon": "🟢", "color": "#4ADE80", "bg": "rgba(74,222,128,0.07)",  "border": "rgba(74,222,128,0.35)"},
    "INFO":     {"icon": "🔵", "color": "#38BDF8", "bg": "rgba(56,189,248,0.07)",  "border": "rgba(56,189,248,0.35)"},
    "ERROR":    {"icon": "⚫", "color": "#64748B", "bg": "rgba(100,116,139,0.07)", "border": "rgba(100,116,139,0.35)"},
}
...
with st.sidebar:
    ...
    ">
        ...
        &nbsp;&nbsp;<span style="color:#7DD3FC">"alert_id"</span><span style="color:#475569">:</span> <span style="color:#86EFAC">"ALT-001"</span>,<br>
        &nbsp;&nbsp;<span style="color:#7DD3FC">"alert_type"</span><span style="color:#475569">:</span> <span style="color:#86EFAC">"Brute Force"</span>,<br>
        &nbsp;&nbsp;<span style="color:#7DD3FC">"description"</span><span style="color:#475569">:</span> <span style="color:#86EFAC">"..."</span>,<br>
        &nbsp;&nbsp;<span style="color:#7DD3FC">"source_ip"</span><span style="color:#475569">:</span> <span style="color:#86EFAC">"x.x.x.x"</span>,<br>
        &nbsp;&nbsp;<span style="color:#7DD3FC">"hostname"</span><span style="color:#475569">:</span> <span style="color:#86EFAC">"server-01"</span><br>
...
if uploaded_file is None:
    ...
    ">
        ...
        <div style="font-family:'Inter',sans-serif; font-weight:500; color:#CBD5E1; font-size:15px; margin-bottom:6px;">
            Awaiting Alert Data
        </div>
        <div style="color:#475569; font-size:13px;">
            Upload a JSON file from the sidebar, or run on the built-in sample alerts
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    ✓ &nbsp;Loaded <strong>{len(alerts)} alerts</strong> — ready for triage
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.get("run_triage"):
    ...
    st.markdown(f"""
    <div style="font-family:'Inter',sans-serif; font-size:14px; color:#38BDF8;
...
if st.session_state.get("results"):
    ...
    for r in results:
        ...
        with st.expander(f"{cfg['icon']}  [{sev}]  {r.get('alert_id')}  —  {r.get('original_type')}"):
            st.markdown(f"""
            <div style="
                background:{cfg['bg']};
            ...
            if str(r.get("is_false_positive","")).lower() == "true":
                ...
                font-family:'Inter',sans-serif;">
                ⚠ &nbsp;Possible False Positive — {r.get('false_positive_reason','N/A')}
                </div>
                """, unsafe_allow_html=True)
